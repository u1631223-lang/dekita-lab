...
## 8. ババ抜きモジュール (`front/src/modules/old-maid`)

| ID | タイトル | ステータス | 担当 | 更新日 | 受け入れ基準 | メモ |
|----|-----------|------------|------|--------|---------------|------|
| OMD-01 | ババ抜き初期実装 | IN PROGRESS | Codex | 2024-05-14 | Old Maid GameModule・Reactスクリーン・テストが揃い、ハブから遊べる | Joker判定・ヒント・Adaptive連携を含む |
...
| NGM-03 | カードスプリント設計・実装 | DONE |  | 2025-10-28 | スピード系カードゲーム実装、AIレベル1-100の設計とロジック実装 | GameModule+画面+ルーティング統合完了 |
| NGM-04 | カードスプリント仕様リバイス | TODO |  | 2025-10-28 | `docs/game-concepts.md` にヒント/デッキ/シャッフル条件が反映され、保護者UX配慮が整理されている | Claude提出物レビュー結果を反映し、追加ガードレールをセット |
| NGM-05 | カードスプリントGameModule実装 | TODO |  | 2025-10-28 | GameModule・画面・`registerModules.ts` への統合、AI難易度1-100が仕様準拠 | Claudeコード取り込み・アクセシビリティ/演出チェックを実施 |
| NGM-06 | 神経衰弱モジュール仕様＆実装 | DONE |  | 2025-10-29 | `docs/shinkeisuijaku.md` の仕様を反映した GameModule とプレイ画面を Hub から起動できる | Concentration（トランプペア）をJP/ENテキストつきで追加 |
